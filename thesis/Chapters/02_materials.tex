...
entanto, podem se tornar altamente produtivos uma vez feitas a calagem e a correção de fósforo,
potássio, enxofre e micronutrientes \citep[p.~617]{yamada_cerrado_2005}. Nas últimas décadas, o bioma tornou-se um dos principais territórios de grãos,
fibra e pecuária do Brasil --- precisamente o que torna a questão do potencial \emph{versus} uso
atual (QP2) relevante aqui.
...
$-19{,}50^\circ$ a $-12{,}39^\circ$. Para a fase de uso atual da terra, as próprias 247
unidades municipais dessa mesma malha alimentam a agregação e o relato municipal.

\section{Os sete segmentos de uso da terra}

O atlas avalia sete segmentos, escolhidos de forma a cobrir produção agrícola, fibra, proteína animal, energia e
...
1 & \textbf{Soja} & Soja mecanizada de sequeiro --- a principal cultura comercial da região,
  cujo desempenho responde fortemente a covariáveis edafoclimáticas como disponibilidade hídrica do
  solo, temperatura e fotoperíodo, como mostram ensaios multiambiente em Mato Grosso
  \citep[pp.~1--3]{corbellini_geographical_2024}, e que já se altera sob as projeções de mudança
  climática: no Arco Norte, a área de soja projetada para 2041--2060 recua em Goiás entre 12,6\% e
  34,8\% \citep[p.~11]{souza_climate-driven_2026}. \\
...
4 & \textbf{Piscicultura} & Aquicultura em tanques (açudes) e reservatórios (espécies de água quente,
  como a tilápia), seguindo critérios de viabilidade local (temperatura --- aqui a do ar, como
  aproximação da temperatura da água ---, textura do solo, declive, acesso à água) estabelecidos para
  aquicultura em viveiros em outras regiões \parencites{ssegane_geospatial_2012}[tab.~1]{assefa_gis_2018}. \\
5 & \textbf{Pecuária} & Gado de corte em pastagem cultivada, que cobre cerca de 28\% do bioma Cerrado
  \citep[p.~1219]{vieira_classification_2022} e abrange um amplo espectro de estados de intensificação
  e degradação \citep{vieira_classification_2022,molossi_improve_2020}. \\
6 & \textbf{Conservação} & Conservação de floresta e Cerrado --- valor de serviços da vegetação nativa,
  ...
  mapeadas para o bioma \citep{schuler_spatial_2022}. \\
7 & \textbf{Geração fotovoltaica} & Geração fotovoltaica em escala de concessionária, com escolha de
  sítio por irradiância, orientação da vertente, temperatura, declive, cobertura da terra e
  acessibilidade, em linha com os fatores de análises multicritério em SIG para usinas fotovoltaicas
  (irradiância, orientação, temperatura de superfície, uso da terra e distância a estradas;
  \citealp[tab.~2]{rane_gis-based_2024}). \\
\bottomrule
\end{tabular}
\end{table}
...
A análise é executada inteiramente no lado do servidor no Google Earth Engine, plataforma em nuvem que
integra um catálogo de dados de observação da Terra a um serviço de computação paralela
\citep[pp.~18--19]{gorelick_google_2017}, utilizada aqui na modalidade gratuita de uso não comercial e
de pesquisa.
...
\begin{tabular}{@{}p{3.2cm}p{3.8cm}p{6.3cm}@{}}
\toprule
...
%% TODO faltam citações para alguns conjuntos de dados, e.g. Accesibility to Cities, HydroSHEDs, WDPA, carbono de biomassa etc
...
\textbf{biofísicas e agroclimáticas} (Tabela~\ref{tab:src-biophys} do Anexo) cobrem temas sobre o
terreno (SRTM); a hidrografia (HydroSHEDS); o clima e o regime de água
através do conjunto de dados TerraClimate, mensal, a $\tfrac{1}{24}^\circ$ ($\approx$4~km)
\citep{abatzoglou_terraclimate_2018}. Cobrem também solos com as camadas OpenLandMap v0.2 a 250~m
(média das profundidades de 0, 10 e 30~cm), % TODO(Zotero): citar Hengl (2018), camadas OpenLandMap SOL v02 a 250 m, quando o PDF/registro estiver no Zotero
água superficial (JRC Global Surface Water) \citep{pekel_high-resolution_2016}, fenologia através do produto MODIS NDVI e
acessibilidade ao mercado (Global Accessibility to Cities). Completam o conjunto a cobertura genérica da terra
do MapBiomas (coleção~10), as áreas protegidas (WDPA), o carbono de biomassa acima do
solo (Spawn \emph{et al.}) e o clima futuro (NASA NEX-GDDP-CMIP6, \citealp{thrasher_nasa_2022}).
...
terra do MapBiomas \citep{souza_reconstructing_2020} e os produtos de
produtividade NPP/GPP do MODIS MOD17 \citep{zhao_improvements_2005}. O uso atual da terra e o indicador de produtividade
...
localmente. Nenhuma técnica de aprendizagem profunda é utilizada.

O código-fonte completo --- configurações, módulos de análise e ferramentas de exportação
--- está disponibilizado publicamente no repositório \texttt{goias\_suitability}
\citep{da_silva_goias_suitability_2026}. O atlas interativo construído a partir destes resultados está publicado em
