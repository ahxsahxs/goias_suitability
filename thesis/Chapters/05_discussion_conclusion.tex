...
jamais tenha informado o modelo: a soja atual alcança 32\% do planalto de argila alta (Zona~1)
...
apta). A discriminação por presença é forte para a soja sobre uma amostra estratificada
espaço$\times$segmento (AUC~0,871, Boyce~0,964).
...
--- aponta para intensificação, não para abertura de novas áreas, o que é coerente com a
...
\begin{itemize}
  ...
  \item \textbf{Descompasso de escala clima--solo.} As entradas climáticas ($\approx$4,6~km,
    ...
    0,13 da variância territorial média, mas essa média esconde dois extremos. Para a geração
    solar, a irradiância (\texttt{clim\_srad}, 0,36) é um sinal climático genuíno --- a única
  ...
  \item \textbf{Sobrepermissividade residual das culturas.} As superfícies de culturas
    permanecem mais permissivas que o nicho realizado ($\kappa=+0{,}269$ para a soja, área
    viável 52,8\% contra 21,6\% do classificador orientado por dados). Separar regra especialista
    larga de lacuna potencial \emph{versus} realizado exigiria, ainda assim, um alvo de
    calibração ancorado em rendimento, que este desenho não usa.
