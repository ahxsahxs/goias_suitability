...
O zoneamento responde à QP1: o agrupamento descorrelacionado resolve GO~e~DF em sete zonas
biofisicamente distintas, que se agrupam em cinco vocações comparativas --- dois planaltos de
cultivo (Zonas~1 e~6, a primeira com maior viabilidade de culturas e cultivo atual mais denso);
um planalto alto e argiloso mais adequado à pecuária extensiva (Zona~3); duas planícies quentes
e de alta irradiância voltadas à geração fotovoltaica (Zonas~2 e~5, esta última terra marginal);
uma baixada ripária voltada à aquicultura (Zona~4); e um planalto alto, íngreme e rico em
carbono onde a viabilidade de culturas colapsa e a conservação é o melhor uso (Zona~7, com
78\% de Cerrado nativo). Essa partição usa apenas atributos biofísicos --- nunca a cobertura da
terra --- e ainda assim reproduz a geografia observada do uso do território, o que sustenta sua
plausibilidade. O padrão subjacente é que GO~e~DF é climaticamente quase uniforme: quem separa
terra boa de terra ruim é o solo e o terreno, não o clima. Essa leitura carrega, portanto, o peso
de dois produtos globais herdados sem validação local: o solo vem do OpenLandMap a 250~m, e o
terreno do SRTM GL1 bruto a 30~m \citep{Farr2007}, reamostrado para a mesma grade; é a acurácia
desse par de camadas --- não a do clima --- que se propaga diretamente para as fronteiras finas
entre zonas (detalhado em~\S\ref{sec:limitations}).
...
planície quente já marginal, enquanto os planaltos privilegiados permanecem essencialmente
inalterados. Três segmentos são mais expostos. A conservação é o mais afetado: sua área viável
cai de 33,2\% para 11,4\% no cenário--janela mais quente, um resultado coerente com o fato de a
vegetação nativa ser o uso da terra menos amortecido pelo clima, ao contrário das culturas,
mantidas perto de seu ótimo térmico pelo manejo. As outras culturas anuais (milho, algodão,
sorgo) recuam de 64,2\% para 59,2\%, e a geração fotovoltaica, de 27,3\% para 17,4\% por perda de
desempenho térmico do painel. Soja e cana-de-açúcar permanecem praticamente intactas.

A fase de uso atual da terra responde à QP2 e, de quebra, valida a QP1 sem que o uso da terra
jamais tenha informado o modelo: a soja atual alcança 28\% do planalto privilegiado contra 3\%
da planície marginal, e as culturas ocupam terra sistematicamente mais viável que a média
disponível (soja: 0,93 nos pixels plantados contra 0,77 na terra apta). A discriminação por
presença é forte para a soja sob validação cruzada com blocos espaciais \citep{Valavi2019}
(AUC~0,83, média por bloco~0,79).

Duas tensões merecem registro. Primeira, uma lacuna substancial entre potencial e uso atual ---
48\% do território é viável para soja e ainda não cultivado, majoritariamente sobre pastagem ---
aponta para intensificação, não para abertura de novas áreas, o que é coerente com a trajetória
já documentada para a fronteira agrícola do Cerrado \citep{Spera2017,Caballero2023}. Segunda, a
verificação cruzada orientada por dados encontra o mapa baseado em conhecimento mais permissivo
que o nicho realizado ($\kappa=0{,}23$ em amostra; $\kappa=0{,}07$ sob validação cruzada
espacial, com o nicho de dados restrito a 7\% do território). Como o clima é quase uniforme, os
...
Por que esse $\kappa$ cai tanto enquanto a AUC de presença continua alta (0,83) é explicado em
\S\ref{sec:validation-present}: são, no fundo, duas perguntas diferentes sobre o mesmo resultado.
...
para lavoura ou pecuária mais intensiva tem um custo de entrada --- calagem e correção da acidez
do solo, mecanização, acesso a crédito --- que a viabilidade biofísica, por si só, não precifica;
...
plausíveis para explicar a lacuna de 48\% aqui observada, mas nenhum deles é modelado neste
...
PCA antes do agrupamento \citep{Ding2004} resolveu as sete zonas relatadas. E reutilizar a mesma
...
como defeito a esconder.

\section{Limitações}\label{sec:limitations}

Parte das limitações abaixo decorre da escolha deliberada por um desenho leve, sem
\emph{upload} de dados e reexecutável a partir do catálogo público; outras são lacunas genuínas
...
\begin{itemize}
  ...
  \item \textbf{Solo e terreno de produtos globais únicos.} A decomposição por fator mostra que,
    ...
    (\S\ref{sec:interpretation}); o atlas herda, portanto, a acurácia de dois produtos globais que
    o desenho sem \emph{upload} impede de validar contra um levantamento local. O solo vem do
  ...
  \item \textbf{Descompasso de escala clima--solo.} As entradas climáticas ($\approx$4,6~km,
    ...
    sua climatologia (1960--1990) não cobre a normal 1991--2020 usada neste trabalho e falta-lhe
    PET/VPD/umidade do solo; CHIRPS/CHIRTS cobrem só uma variável cada e, a 0,05\textdegree{}
    ($\approx$5,6~km), são \emph{mais} grosseiros que o TerraClimate; ERA5-Land/AgERA5 têm o
    conjunto completo de variáveis e o período correto mas ficam a 0,1\textdegree{}
    ($\approx$9--11~km, mais de 2$\times$ pior), e o AgERA5 sequer está no catálogo oficial. Que
    o clima atue como comporta e não como fonte de estrutura fina não é apenas uma leitura
    qualitativa: a variabilidade espacial local (desvio-padrão em janela móvel, bandas
    padronizadas) de terreno+solo é 42$\times$ maior que a do clima mesmo na janela de 4,75~km
    --- a própria escala nativa do TerraClimate, a mais favorável possível a ele --- e o padrão é
    quase uniforme em todo o território (Figura~\ref{fig:scale-mismatch},
    Tabela~\ref{tab:scale-mismatch} do Anexo). A resolução nominal do clima é, portanto,
    enganosa apenas em aparência: o impacto prático é pequeno porque a estrutura fina do atlas
    nunca dependeu dela. Essa leitura --- clima \emph{versus} terreno+solo como dois blocos ---
    simplifica, porém, a estrutura real do modelo se aplicada ingenuamente por \emph{prefixo de
    origem do dado} em vez de por \emph{papel temático}: agregando a decomposição por fator
    (Figura~\ref{fig:factor-variance}) por tema, e não só por segmento isolado, o clima explica
    0,15 da variância territorial média, mas essa média esconde dois extremos. Para a geração
    solar, a irradiância (\texttt{clim\_srad}, 0,46) é um sinal climático genuíno --- a única
    variável verdadeiramente espacial de origem climática que domina um segmento. Já para a
    pecuária, o fator dominante nominalmente rotulado ``clima'' é a umidade do solo do TerraClimate
    (\texttt{clim\_soil\_moist}, 0,58): tematicamente, essa variável descreve armazenamento de
    água no solo, não um gradiente climático espacial, e vem rotulada ``clima'' apenas porque o
    produto de origem é um balanço hídrico climático. Reclassificando-a pelo papel temático em vez
    do prefixo da banda, a parcela de terreno+solo da pecuária sobe de 0,42 para 1,00 e sua parcela
    climática cai a zero --- coerente com o restante das culturas. A nomenclatura de origem do
    dado, portanto, não deve ser confundida com o papel temático da variável no modelo: é a
    padronização (\S\ref{sec:fuzzy-standard}) e a estrutura de agregação por fator que definem o
    que conta como ``clima'', não o catálogo de onde a banda foi extraída.
  \item \textbf{Sobrepermissividade residual das culturas.} As superfícies de culturas
    permanecem mais permissivas que o nicho realizado ($\kappa=0{,}23$ em amostra;
    $\kappa=0{,}07$ sob validação espacial). Separar regra especialista larga de lacuna
    potencial \emph{versus} realizado exigiria, ainda assim, um alvo de calibração ancorado em
    rendimento, que este desenho não usa.
  ...
  \item \textbf{Piscicultura tem uso realizado quase inexistente no MapBiomas.} A revisão de
    2026-09 dos limiares e pesos (\S\ref{sec:ahp}) encontrou apenas um único pixel de aquicultura
    mapeado pelo MapBiomas em todo o território GO~e~DF --- amostra insuficiente para ancorar
    limiares ou validar presença contra esse uso realizado. A calibração da piscicultura
    permanece ancorada na terra genericamente \emph{disponível}, não no nicho realizado
    (\texttt{tools/anchor\_breakpoints.py}), e nenhum validador de presença (Boyce/AUC) é
    reportado para esse segmento pela mesma razão --- uma lacuna genuína, não um problema de
    metodologia corrigível dentro do desenho GEE-nativo atual.
  \item \textbf{Fronteiras municipais e projeção futura.} O limite administrativo usado é mais
    grosseiro que uma malha municipal de precisão nacional. E a projeção CMIP6, como qualquer
...
Também fica pendente publicar os entregáveis (como o código fonte, um atlas interativo, um \emph{ranking}
municipal de oportunidade/vulnerabilidade), para que estes sejam exploráveis
e reexecutáveis para outros estados do Cerrado. Por fim, cabe revisar os critérios de
...
território em sete zonas que reproduzem, de forma independente, a geografia observada do uso da
