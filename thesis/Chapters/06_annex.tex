...
plana (0,17--0,19) e o Davies--Bouldin fica próximo em $k=7$, $8$ e $10$, de modo que nenhum
dos dois decide sozinho; $k = 10$ (linha tracejada) combina a silhueta mais alta do intervalo
(0,187) com a estabilidade de longe mais alta sob subamostragem (ARI $=0{,}962$, contra
$0{,}687$ em $k=7$). Um \emph{sweep} estendido a $k=2\dots20$ (não plotado) confirma que esse
pico de silhueta não é um artefato de faixa truncada.}
...
e solo dominam as culturas, mas o fator vencedor não é sempre o mesmo: declive responde por
0,78 da variância da soja (argila, 0,22) e por $\approx$1,0 de outras culturas anuais, ao passo
que para a cana-de-açúcar é a argila que domina (0,69, contra 0,31 do declive). Para soja e
...
radiação solar (0,36). A concentração em um ou dois fatores por segmento não decorre de peso
AHP enviesado a favor deles --- pelo contrário: a revisão de pesos de 2026-09
(\S\ref{sec:ahp}) deslocou peso deliberadamente \emph{para} os fatores de maior poder
discriminante em cada segmento, e mesmo assim as razões máx./mín. de peso dos sete segmentos
ficaram todas na mesma ordem de grandeza pós-recalibração (2,2--4,3$\times$;
Tabela~\ref{tab:spec-longtable}), sem o contraste que uma calibração enviesada precisaria
mostrar entre um segmento artificialmente concentrado e outro genuinamente equilibrado. Que
dar mais peso justamente aos fatores mais discriminantes não tenha reduzido a concentração de
variância é evidência de que a saturação é geográfica --- as pertinências climáticas em GO/DF
saturam genuinamente, como evidenciado independentemente pelo mapa de magnitude espacial da
Figura~\ref{fig:scale-mismatch} --- e não um artefato de como os pesos foram elicitados.}
...
construído a partir da mesma amostra limitada por orçamento computacional (4.475 pontos) que
...
conservação (\S\ref{anx:derivations}). Amostra de 4.475 pontos por raio, limitada por
