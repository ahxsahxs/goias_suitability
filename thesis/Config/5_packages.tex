...
% \restylefloat{figure}     % DISABLED: redefines figure-caption internals (@fs@capt) in a way
                             % that conflicts with rotating's \sidewaysfigure (Undefined control
                             % sequence at \caption time); never needed anything from it besides
                             % the [H] placement specifier, which \usepackage{float} alone provides.
...
\usepackage{rotating}     % \sidewaysfigure / \sidewaystable (full-page landscape floats)
...
\ifPDFTeX
  ...
  % NOTA: NÃO carregar lmodern aqui -- ele redefine as fontes de símbolos/extensão
  % matemáticas (OMS/OMX) para Latin Modern *depois* que newpxmath (carregado pela
  % classe via FontStyles/newpx.ldf) já as configurou para Palatino/newpx, causando
  % glifos ausentes (parênteses grandes, \sum, ≤, ≥, ...) em todo o documento.
  % newpxtext já fornece uma fonte de texto T1 completa, então lmodern é
  % desnecessário aqui.
